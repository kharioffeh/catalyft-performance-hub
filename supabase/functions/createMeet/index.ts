...
serve(async (req) => {
  ...
  try {
    ...
    const { title, description, start_ts, end_ts } = await req.json();

    // Validate required fields
    if (!title || typeof title !== 'string') {
      return new Response(
        JSON.stringify({ error: 'Meet title is required' }),
    ...
    if (!start_ts || !end_ts) {
      return new Response(
        JSON.stringify({ error: 'Start time and end time are required' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    console.log(`User ${user.id} creating meet: ${title}`);
    ...
    console.log(`Successfully created meet ${meet.id}`);

    // Publish event after successful DB operation
    const uid = user.id;
    publishEvent(uid, "meetCreated", { meet_id: meet.id });
