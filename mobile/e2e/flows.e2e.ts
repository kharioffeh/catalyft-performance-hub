...
const helpers = {
  ...
  async navigateToTab(tabName: string) {
    ...
    await expect(element(by.id(`${tabName.toLowerCase()}-container`))).toBeVisible();
  },
  
  async fillLiftEntry(exercise: string, weight: string, reps: string, sets?: string) {
  ...
  async addMealEntry(name: string, calories: string, protein?: string) {
