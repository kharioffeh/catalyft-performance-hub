...
export default function ReadinessDetailPage() {
  ...
  const [period, setPeriod] = useState<7 | 30 | 90>(periodParam === 7 ? 7 : periodParam === 90 ? 90 : 30);
  ...
  const {
    ...
    isLoading,
    error
  } = useMetricData("readiness", period);
  ...
  return <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          {[7, 30, 90].map(d => <button key={d} className={`px-3 py-1 rounded ${period === d ? "bg-blue-600 text-white" : "bg-gray-200 hover:bg-gray-300"}`} onClick={() => changePeriod(d as 7 | 30 | 90)}>
              {d}d
            </button>)}
        </div>
      </div>
      ...
      <section className="card">
        ...
        <div className="relative w-full pb-[60%]">
          <MetricChart type="line" data={data.series || []} zones={[{
          from: 0,
          to: 33,
          color: "#ef4444",
          label: "Low"
          ...
          color: "#10b981",
          label: "Optimal"
        }]} xLabel="Date" yLabel="Readiness Score" className="absolute inset-0" />
        </div>
      </section>

      {/* Secondary Chart: HRV & Sleep Quality */}
      {data.secondary && data.secondary.length > 0 && <section className="card">
          <h2 className="text-lg font-semibold mb-2">Contributing Factors</h2>
          <p className="text-sm text-gray-600 mb-4">
            HRV and sleep quality metrics that influence readiness
          </p>
          <div className="relative w-full pb-[60%]">
            <MetricChart type="bar" data={data.secondary.map(point => ({
          x: point.x,
          y: 0,
          hrv: point.hrv,
          sleep: point.sleep
        }))} xLabel="Date" yLabel="Value" multiSeries={true} className="absolute inset-0" />
          </div>
        </section>}
