...
import { ProgramMeta } from '@/hooks/useEnhancedProgramBuilder';

interface ProgramMetaStepProps {
  meta: ProgramMeta;
  setMeta: (meta: ProgramMeta) => void;
  onNext: () => void;
}

export const ProgramMetaStep: React.FC<ProgramMetaStepProps> = ({ meta, setMeta, onNext }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-white mb-2">Program Details</h2>
        <p className="text-white/70">Set up the basic information for your training program</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-6 space-y-6">
          ...
          {/* Program Length */}
          <div className="space-y-3">
            <Label className="text-white">Program Length: {meta.weeks} weeks</Label>
            <Slider
              value={[meta.weeks]}
